...
Итак в жаве есть 2 опкода которые работают с булеанами: `IFNE` и `IFEQ`. Первый прыгает если `true` второй если `false`.  
Например для кода 
```java
...
```
Псевдо байт-код будет выглядить так:
```
GETFIELD SideUtil.isServer Z
IFEQ 1
// code
LABEL 1
```
Эвристика поступит следующим образом:
1. Рассматриваем код со стороны клиента
2. `GETFIELD` отмечаем как код существующий на клиенте
3. `IFEQ` отмечаем как код существующий на клиенте
4. Смотрим какое значение попало в `IFEQ` и передаем его на рассматрение в маркеры
5. Один из наших маркеров решил, что клиент в данной ситуации точке прыгнет, а значит не попадет в код, а значит код не будет отмечен как существующий на клиенте
6. Вырезаем весь байт код в который не смогли попасть

Из-за такого подхода возникают интересные ситуации.   
Например есть код  
```java
if (SideUtil.isServer) {
    // server code
...
```
Если вы соберете клиентский билд и декомпильните его, то увидите что то типа
```java
if (SideUtil.isServer) {}
// client code
```
Т.е. клиентский код будет выполнен независимо от того, что скажет `SideUtil.isServer`. Будьте осторожны в подборе маркеров.

Также хочется заметить, что эвристика работает с `&&` и `||`, но не работает с `|` и `&`. 
Также она не работает если вы сначала запишете значение в переменную а потом сунете ее в иф.

Теперь собственно о том как это настраивать.   
Пример конфигурации:
```groovy
...
```
В целом я считаю, что все довольно очевидно, но все же пройдемся по каждому слову.  
  
Эвристика воспринимает как поля так и методы. Соответственно нужно задать
 1. имя класса в котором объявленна функция/поле (`owner`)
 2. имя поля/функции (`name`)
 3. стороны к которым этот маркер принадлежит. т.е. указывая `server` вы говорите вырезалке, что это поле/функция вернет тру только на сервере и что на сервере она всегда будет возвращать тру.
 4. (только для метода) дескриптор метода (`desc`). 
 
Указывать можно какие угодно методы и поля с одним лишь ограничением. Они должны возвращать `boolean`
