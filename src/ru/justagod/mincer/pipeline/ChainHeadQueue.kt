...
class ChainHeadQueue(
        ...
        val input: Any,
        private val archive: MincerArchive?,
        val next: RawChainSegment?
) {

    fun advance(input: Any, fs: MincerFS): ChainHeadQueue? {
        if (next == null) return null
        return ChainHeadQueue(next.pipeline, input, fs.pullArchive(next.pipeline.id), next.next)
    }
