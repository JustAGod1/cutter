...
data class MethodBodyAtom(val owner: ClassTypeReference, val name: String, val desc: String): ProjectAtom() {
    constructor(owner: ClassTypeReference, method: MethodNode): this(owner, method.name, method.desc)
    constructor(method: MethodAtom): this(method.owner, method.name, method.desc)

    override fun path(): String = owner.name + "." + name + desc
    override fun parent(): ProjectAtom = MethodAtom(owner, name, desc)
}
