...
class BytecodeModelFactory(private val harvester: NodesFactory) : ModelFactory {
    ...
    companion object {
        ...
        val nullableAnnotation = ClassTypeReference(Nullable::class.java)
        ...
        fun makeModel(node: ClassNode, parent: AbstractModel?): InternalClassModel {
                ...
                ?.mapValues {
                    it.value.mapValues {
                        ...
                        if (it.value is Array<*>)
                            EnumHolder(
                                fetchTypeReference((it.value as Array<String>)[0]) as ClassTypeReference,
                                (it.value as Array<String>)[1]
                            )
                        else
                            it.value
                    }
            ...
            model._typeParameters =
                { parseClassSignature(node.signature, model).filterIsInstance<ReferencedGenericTypeModel>() }
            val (superClass, interfaces) = if (node.signature != null) {
                ...
                Pair(node.superName?.let { FinalTypeModel(ClassTypeReference(it.toCanonicalName()), model) },
                    node.interfaces
        ...
        private fun makeFieldModel(field: FieldNode, parent: AbstractModel): FieldModel {
            ...
            return FieldModel(
                ...
                (field.invisibleAnnotations
                    ?: emptyList<AnnotationNode>()).any { it.desc == nullableAnnotation.desc() },
                field,
                parent
            )
        }
        ...
        private fun parseSignature(signature: String, parent: AbstractModel, arrayed: Boolean = false): TypeModel {
            ...
            else if (signature == "*") WildcardGenericTypeModel(
                FinalTypeModel(
                    ClassTypeReference("java.lang.Object"),
                    parent
                ), parent
            )
        ...
        private fun parseWildcard(signature: String, parent: AbstractModel): WildcardGenericTypeModel {
            return if (signature.startsWith("-")) WildcardGenericTypeModel(
                FinalTypeModel(
                    ClassTypeReference("java.lang.Object"),
                    parent
                ), parent
    ...
    class BytecodeNotFoundException(val className: String) : Exception(className)
