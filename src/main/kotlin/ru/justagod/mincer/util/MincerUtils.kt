...
object MincerUtils {
    ...
    fun processFolder(
        ...
        folder: File,
        threadsCount: Int = 20,
        listener: ProcessingListener = ProcessingListener()
    ) {
        ...
        executor.shutdown()
        val completed = executor.awaitTermination(5, TimeUnit.MINUTES)
        if (!completed) {
            throw RuntimeException("MincerUtils#processFolder executor could not be stopped in 5 minutes")
        }
    }
    ...
    fun processFolder(
        ...
        do {
            ...
            val queue =
                if (targetClasses == null) getClassesInFolder(folder) else getFilesFromClasses(folder, targetClasses)
                        ...
                        while (f != null) {
                            ...
                            if (listener.filter(ref)) {
                                val result = panel.advance(ref).onModification {
                                    ...
                                    if (!f.delete()) throw RuntimeException("Could not delete file $f")
                                }
    ...
    private fun awaitAndAggregate(futures: List<Future<*>>) {
    ...
    private fun getFilesFromClasses(root: File, classes: List<ClassTypeReference>): LinkedBlockingQueue<File> {
    ...
    fun processFolderRecursively(root: File, factory: (MincerFS) -> Mincer) {
        ...
        for (s in root.listFiles()!!) {
            ...
            if (extension == "jar" || extension == "zip") {
                ...
                archives += MincerZipFS(s, entries)
            }
        }
    ...
    fun processArchive(root: File, archiveFile: File, factory: (MincerFS) -> Mincer) {
        ...
        val archive = MincerZipFS(archiveFile, entries)
    ...
    private fun processArchive(generalFS: MincerFS, archive: MincerZipFS, factory: (MincerFS) -> Mincer) {
        ...
        do {
            archive.entries.map { it.value }.parallelStream().forEach { entry ->
                ...
                if (result.type == MincerResultType.MODIFIED) archive.entries[entry.path] =
                    ByteArraySource(entry.path, result.bytecode())
